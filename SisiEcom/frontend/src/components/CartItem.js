    ...
    <div className='w-full min-h-[150px] flex items-center gap-x-4 '>
      ...
      <div className='w-full flex flex-col'>
        ...
        <div className=' flex gap-x-2 h-[36px] text-sm '>
          ...
          {/* item price */}
          <div className='flex-1 flex items-center justify-around '>{price} dt</div>
          {/* final price */}
          {/* make the price at 2 decimals */}
          <div className='flex-1 flex justify-end items-center text-primary font-medium'>{` ${parseFloat(price * amount).toFixed(2)} dt`}</div>
        </div>
      </div>
    </div>
  </div>
